...
#        raise PreprocessError("detected recursive #include of '%s'" % infile)
        return defines

    file_cur_path = ""
    if outfile and not isinstance(outfile, basestring):
        path = infile
        if len(includePath):
            if path.startswith(includePath[0]):
                path = path[len(includePath[0]):]
        file_cur_path = path
        outfile.write('// >> %s\n' % path)

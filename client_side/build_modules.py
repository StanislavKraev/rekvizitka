...
def usage():
    print("Rekvizitka modules builder v1.0")
...
def load_locale(args, locale, module_name, base_name, incdir):
    locale_dir = os.path.join(incdir, 'locales')
...
def load_templates(args, templates, module_name, all_modules, incdir):
    if not len(templates):
        return ""
    template_dir = os.path.join(incdir, 'templates')
    templates_list = []
...
def build_css(args, css_files, incdir):
    css_list = []
    css_dir = os.path.join(incdir, 'css')
...
def process_module(dir, all_modules):
    ...
    src_path = os.path.join(incdir, src_name)
    ...
    if 'templates' in descr_data:
        module_templates = load_templates(args, descr_data['templates'], descr_data['name'], all_modules, incdir)
    ...
    if 'css' in descr_data:
        css = build_css(args, descr_data['css'], incdir)
    ...
    for dest_dir, mod_name in dest_dirs:
        ...
        module_locale = load_locale(args, locale, mod_name, descr_data['name'], incdir) if has_locale else ""
...
def visit(pattern, dir, files):
    for filename in files:
        if fnmatch(filename, pattern[0]):
            process_module(os.path.join(dir, filename), pattern[1])

def process_modules(folder, all_modules):
    os.path.walk(folder, visit, ('descriptor.json', all_modules))
...
def main():
    ...
    process_modules(args['incdir'], all_modules)

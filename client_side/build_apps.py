...
def usage():
    print("Rekvizitka applications builder 1.0")
...
def get_jslint_assumes():
    ...
    window: false, XMLHttpRequest: false, _: false, $: false, YUI: false, YUI_config:true, console:false, Backbone: false, jQuery: false, qq: false, io: false, Audio:false */
    """
...
def get_mod_sum_version(app_deps):
    sum_maj = 0
    sum_mid = 0
    sum_min = 0
    for mod in app_deps:
...
def process_application(dir, all_modules, tree, modules_json, dynamic_modules_json):
    locale = args['locale']

    descr_path = dir
    incdir = os.path.dirname(dir)
    ...
    main_file_path = os.path.join(incdir, source_file_name)

    main_css_content = ""
    if 'css' in descr_data:
        for css_item in descr_data['css']:
            css_dir_path = os.path.join(incdir, 'css')
    ...
    if 'locales' in descr_data and locale in descr_data['locales']:
        locale_path = os.path.join(incdir, 'locales')
...
""" % {'locale_content' : read_file(locale_path),
    ...
    template_content, template_req = load_app_templates(descr_data.get('templates'), incdir, descr_data['name'])
    if template_req:
    ...
    mod_sum_version = get_mod_sum_version(app_deps)

    app_path = os.path.join(args['outdir'], "%(app_name)s-%(version)s_%(mod_sum_version)s_%(locale)s.js" % {'app_name' : descr_data['name'],
                                                                                        'version' : descr_data['ver'],
                                                                                        'locale' : locale,
                                                                                        'mod_sum_version' : mod_sum_version})

    app_css_path = os.path.join(args['outdir'], "%(app_name)s-%(version)s_%(mod_sum_version)s_%(locale)s.css" % {'app_name' : descr_data['name'],
                                                                                             'version' : descr_data['ver'],
                                                                                             'locale' : locale,
                                                                                             'mod_sum_version' : mod_sum_version})
...
def visit(pattern, dir, files):
    for filename in files:
        if fnmatch(filename, pattern[0]):
            process_application(os.path.join(dir, filename), pattern[1], pattern[2], pattern[3], pattern[4])

def process_apps(folder, all_modules, tree, modules_json, dynamic_modules_json):
    os.path.walk(folder, visit, ('descriptor.json', all_modules, tree, modules_json, dynamic_modules_json))
...
def main():
    ...
    process_apps(args['incdir'], all_modules, tree, modules_json, dynamic_modules_json)
